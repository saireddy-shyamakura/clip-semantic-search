"""
pipeline.py — V3 search pipeline orchestrator for Momento.

Multi-stage retrieval pipeline:
1. Query classification (router)
2. Exact index lookup (filename/path FTS)
3. Query expansion (synonym-based)
4. Multi-variant recall from vector index
5. Optional re-ranking
6. Cross-modal fusion (embedding + YOLO + OCR)
7. Threshold & trim
"""
...
from ..embedding.legacy_compat import extract_image_features, extract_text_features
...
from ..core.validation import validate_image_path, validate_text_query, ValidationError
